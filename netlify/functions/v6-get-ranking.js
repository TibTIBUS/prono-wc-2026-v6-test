...
async function computeLeaderSince(db, currentLeaderEmployeeId, currentLeaderName) {
  ...
  for (const row of rows) {
    ...
    if (!leadersByDate.has(d) && Number(row.rank) === 1) leadersByDate.set(d, row);
  }
...
function computeMovers(ranking) {
  ...
  return {
    ...
    } : { employee: "-", movement: 0, label: "Aucun mouvement" },
...
exports.handler = async () => {
  try {
    ...
    const ranking = employees.map(employee => {
      ...
      let groupTotal = 0, knockoutTotal = 0, exact = 0, good = 0;
    ...
    return json(200, { ranking, classement: ranking, rows: ranking, players: ranking, meta, stats: meta, summary: meta, fun_stats });
  } catch (error) {
    return json(500, { error: error.message || "Erreur serveur classement V6.7" });
  }
};
